...
const StepSuccess = ({ lang, t, formData }) => {
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <style>
          ...
          .receipt-container { max-width: 850px; margin: 0 auto; background: var(--card); border-radius: 999px; box-shadow: 0 40px 100px -20px rgba(0,0,0,0.1); overflow: hidden; border: 1px solid #e2e8f0; }
          ...
          .logo-area { font-size: 38px; font-weight: 700; letter-spacing: -1.5px; }
          .status-badge { background: rgba(255,255,255,0.2); backdrop-filter: blur(10px); padding: 10px 24px; border-radius: 12px; font-size: 14px; font-weight: 700; border: 1px solid rgba(255,255,255,0.3); }
          ...
          .section-title { font-size: 18px; font-weight: 700; color: var(--primary); text-transform: uppercase; }
          ...
          .tag { background: #f1f5f9; color: var(--text); padding: 6px 14px; border-radius: 12px; font-size: 12px; font-weight: 700; border: 1px solid #e2e8f0; }
          ...
          .schedule-item { border: 1px solid #eee; padding: 12px; border-radius: 12px; text-align: center; background: white; }
              ...
              <div class="schedule-grid">
                ${Object.entries(formData.workingHours).map(([day, conf]) => `
                  <div class="schedule-item ${conf.closed ? 'closed' : ''}">
                    <div style="font-weight: 700;">${t.hours.days[day] || day}</div>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 px-4 text-center animate-in zoom-in-95 fade-in duration-500">
      ...
      <div className="relative mb-12">
        <div className="absolute inset-0 bg-[var(--primary-a20)] dark:bg-[var(--primary-a20)] blur-[100px] rounded-full animate-pulse"></div>
        <div className="relative z-10 w-40 h-40 bg-[var(--primary)] text-white rounded-[3rem] flex items-center justify-center shadow-[0_25px_60px_rgba(143,92,177,0.4)] rotate-3 animate-bounce-slow">
        ...
        <div className="absolute -bottom-6 -right-6 glass p-4 rounded-3xl shadow-2xl animate-in slide-in-from-bottom-4">
            <ShieldCheck className="text-[var(--primary-light)]" size={32} />
        </div>
      </div>

      <div className="space-y-10 max-w-2xl mx-auto">
        <div className="space-y-4">
          <h2 className="text-4xl sm:text-6xl font-bold text-[var(--primary)] dark:text-white tracking-tight leading-tight">
            {t.success.title}
          </h2>
          <div className="flex items-center justify-center gap-2">
             <div className="h-1.5 w-12 bg-[var(--primary)] rounded-full"></div>
             <div className="h-1.5 w-1.5 bg-[var(--primary-a40)] rounded-full"></div>
             <div className="h-1.5 w-1.5 bg-[var(--primary-a40)] rounded-full"></div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6">
          <div className="p-8 rounded-[2.5rem] glass text-right rtl:text-right ltr:text-left flex flex-col gap-4 hover:border-[var(--primary-a40)] transition-all shadow-xl group cursor-default">
             <div className="w-14 h-14 bg-[var(--primary-a20)] rounded-2xl flex items-center justify-center text-[var(--primary-light)] group-hover:scale-110 group-hover:rotate-6 transition-all">
                <Award size={32} />
             </div>
             <div>
        ...
        <div className="pt-10 flex flex-col sm:flex-row gap-5 justify-center">
          <button 
            ...
            className="group relative px-14 py-5 bg-gradient-to-r from-[var(--primary)] to-[#6d3a91] text-white font-bold rounded-[2rem] shadow-[0_20px_40px_rgba(143,92,177,0.3)] hover:shadow-[var(--primary-a50)] hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-4 overflow-hidden"
          >
          ...
          <button 
            ...
            className={`
              relative px-14 py-5 glass hover:bg-[var(--primary-a10)] text-[var(--primary)] dark:text-white font-bold rounded-[2rem] transition-all hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-4 shadow-xl group border border-[var(--primary-a30)]
              ${isDownloading ? 'opacity-70 cursor-wait' : ''}
            `}
          >
            {isDownloading ? (
              <Loader2 size={24} className="animate-spin text-[var(--primary-light)]" />
            ) : (
              <Download size={24} className="text-[var(--primary-light)] group-hover:-translate-y-1 transition-transform" />
            )}
